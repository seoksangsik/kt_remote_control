...
import com.kt.remotecontrol.model.Response;
...
public class NanoHTTPD {
    ...
    private static final Log LOG = new Log("NanoHTTPD");
    ...
    public Response serve(String uri, String method, Properties header, Properties params) {
        ...
        LOG.message("serve, method = " + method + " '" + uri + "' ");

        Enumeration e = header.propertyNames();
        while (e.hasMoreElements()) {
            String value = (String) e.nextElement();

            LOG.message("Header: '" + value + "' = '" + header.getProperty(value) + "'");
        }
        ...
        while (e.hasMoreElements()) {
            ...
            LOG.message("Param: '" + value + "'='" + params.getProperty(value) + "'");
        }

        String result = serverManager.processRequest(uri, params);

        Response response = new Response(HTTP_OK, MIME_HTML);
        if (result != null) {
            response.setData(result);
        }

        LOG.message("http request method : " + method);
        LOG.message("response : " + response.getData());
     ...
     */
    public NanoHTTPD(int port, ServerManager serverManager) throws IOException {
        myTcpPort = port;
        this.serverManager = serverManager;

        ss = new ServerSocket(myTcpPort);
        ss.setSoTimeout(Integer.MAX_VALUE);

        LOG.message("***********************************");
        LOG.message("ServerSocket ADDRESS : " + ss.getInetAddress());
        LOG.message("ServerSocket PORT : " + ss.getLocalPort());
        LOG.message("ServerSocket TIMEOUT : " + ss.getSoTimeout());
        LOG.message("***********************************");
...
//                        LOG.message("wait for client connected");
                    try {
                        ...
                        LOG.message("accept, address=" + socket.getInetAddress());

                        if (ignoreRequest(socket.getInetAddress())) {
                            LOG.message("ignoreRequest!");
                            return ;
                        }

                        HTTPSession session = new HTTPSession(socket);

                        LOG.message("### one client connected, http session = " + session.toString());
                    } catch (SocketTimeoutException ste) {
//                            LOG.message("Accept timed out");
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                } // while

                LOG.message("NanoHTTPD thread End, now it is impossible to server remote control");
            }

            // 리모트 서버에서 온게 아니면 거부 하라
            private boolean ignoreRequest(InetAddress inetAddress) {
                if (!WorkingConfig.CHECK_IFSERVER_REQUEST) {
                    return false;
                }

                InetAddress address = null;
                try {
                    address = InetAddress.getByName(WorkingConfig.CONTROL_SERVER);
                } catch (UnknownHostException e) {
                    e.printStackTrace();
                }

                LOG.message("address(" + inetAddress);

                if (inetAddress.equals(address) || inetAddress.equals(WorkingConfig.LOCAL_SERVER)) {
                    LOG.message("address(" + inetAddress + ") is same from REMOTE SERVER : " + address);
                    return false;
                }

                LOG.message("address(" + inetAddress + ") is different from REMOTE SERVER : " + address);

                return true;
            }
        }, "NanoHttpd Listen thread");
        t.start();
    ...
    private class HTTPSession implements Runnable {
        ...
        public void run() {
            ...
            LOG.message("httpd session START >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");

            try {
                InputStream is = mySocket.getInputStream();

                if (is == null) {
                    LOG.message("error input stream is null, return");
                    return;
                }

                BufferedReader in = new BufferedReader(new InputStreamReader(is));

                LOG.message("Read the request line");

                // Read the request line
                String readLine = in.readLine();
                LOG.message("readLine : " + readLine);
                ...
                int qmi = uri.indexOf('?');
                if (qmi >= 0) {
                    decodeParms(method, uri.substring(qmi + 1), params);
                ...
                if (method.equalsIgnoreCase("POST")) {
                    LOG.message("method is post");

                    int size = 0x7FFFFFFF;// FFFFFFFFl;
                    ...
                    String postLine = "";
                    // -------------------- content length 가 실제 보다 많게 설정되어 오는 것 때문에 고친 부분
                    char buf[] = null;
                    if (size != 0x7FFFFFFFFFFFFFFFl) { // size
                        LOG.message("content size = " + size);
                        buf = new char[size];
                    } else { // size 를 1024 로 줄인다. 명령은 그렇게 크지 않다.
                        buf = new char[1024];
                    }

                    int read = in.read(buf);
                    LOG.message("read size = " + read);
                    postLine = String.valueOf(buf, 0, read);
                    // -------------------- content length 때문에 고친 부분 끝..

                    postLine = postLine.trim();

                    LOG.message("end postLine[" + postLine + "]");

                    decodeParms(method, postLine, params);
                }
         ...
         * Properties.
         */
        private void decodeParms(String method, String parms, Properties p) throws InterruptedException {
            if (parms == null)
                return;
        ...
        private void sendResponse(String status, String mime, Properties header, InputStream data) {
            try {
                ...
                LOG.message("HTTP/1.0 " + status + " \r\n");

                if (mime != null) {
                    pw.print("Content-Type: " + mime + "\r\n");

                    LOG.message("Content-Type: " + mime + "\r\n");
                }

                if (header == null || header.getProperty("Date") == null) {
                    pw.print("Date: " + gmtFrmt.format(new Date()) + "\r\n");

                    LOG.message("Date: " + gmtFrmt.format(new Date()) + "\r\n");
                }
