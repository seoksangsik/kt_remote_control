...
acba.define('js:Popup',
    ['log:kids(Popup)', 'js:nav_util', 'js:Enum', 'js:Config', 'js:network',
    'js:NavCoreProxy', 'js:KidsProxy', 'js:AppProxy'],
function(log, util, Enum, Config, network,
         NavCoreProxy, KidsProxy, AppProxy) {
    ...
    function setVisible(visible, activate) {
        ...
        if (visible) {
            if (activate && (isDataChannel() || AppProxy.isBoundApp())) {
    ...
    function loadPicture(url) {
        ...
        pictureImg.onload = function() {
            ...
            if (pictureImg.width && pictureImg.height) {
                ...
                showImageError(ERROR_CODE.IMAGE_OTHER_ERROR_422);
            }
        };
    ...
    function getErrorMessage(type) {
        ...
        case ERROR_CODE.IMAGE_TOO_MUCH_SIZE_419 :
            message = '수신된 사진의 사이즈가 커서 표시할 수 없습니다. 전송한 사진의 크기를 줄이신 후 다시 시도해 주세요.';
            break;
        case ERROR_CODE.IMAGE_UNSUPPORT_FORMAT_420 :
            message = '수신된 사진의 포맷을 지원하지 않습니다. 지원되는 포맷은 JPG와 PNG입니다. 다시 확인해 주세요.';
            break;
        case ERROR_CODE.IMAGE_UNSUPPORT_MODEL_421 :
            message = '사진 보기 서비스가 지원하지 않는 단말입니다.';
            break;
        case ERROR_CODE.IMAGE_OTHER_ERROR_422 :
    ...
    function makePictureTagExpandW(pictureImg, pictureWidth, pictureHeight) {
        ...
        var popupDiv, bgDiv, strokeDiv, buttonDiv,
            ...
            shotcutHeight = 0,
            bottonAttrValue,
            styleName = POPUP.STYLE.PICTURE + CHAR.UNDERSCORE + POPUP.BG_EXPAND.WIDTH;
